...
class GITBLEND_OT_Checkout(bpy.types.Operator):
    """Git Blend Checkout Operator - Restore scene to a specific commit"""
    ...
    def poll(cls, context):
        """Check if checkout operation is possible"""
    ...
    def execute(self, context):
        """Main execution method for checkout operation"""
        # Setup paths
        blend_path = Path(bpy.data.filepath).resolve()
        project_dir = blend_path.parent
        gitblend_dir = project_dir / ".gitblend"
        ...
        # Validation
        if not self._validate_checkout_prerequisites(gitblend_dir, metadata_file):
            return {'CANCELLED'}
        
        try:
            # Load and validate commit metadata
            metadata = self._load_metadata(metadata_file)
            target_commit = self._find_target_commit(metadata, self.commit_hash)
            if not target_commit:
                return {'CANCELLED'}
            
            # Build commit chain for reconstruction
            commit_chain = self._build_commit_chain(metadata['commits'], target_commit)
            if not commit_chain:
                self.report({'ERROR'}, "Failed to build commit chain")
                return {'CANCELLED'}
            
            # Load target signature for validation
            ...
            # Perform scene reconstruction
            self._perform_scene_reconstruction(gitblend_dir, commit_chain, target_signature)
            
            # Post-reconstruction tasks
            self._perform_post_reconstruction_tasks(context, target_commit)
    ...
    def _validate_checkout_prerequisites(self, gitblend_dir, metadata_file):
        """Validate that checkout operation can proceed"""
    ...
    def _perform_scene_reconstruction(self, gitblend_dir, commit_chain, target_signature):
        """Orchestrate the complete scene reconstruction process"""
    ...
    def _perform_post_reconstruction_tasks(self, context, target_commit):
        """Handle post-reconstruction UI updates and cleanup"""
    ...
    # ============================================================================
    # COMMIT CHAIN AND METADATA HANDLING
    # ============================================================================
    
    def _load_metadata(self, metadata_file):
        """Load commit metadata from file"""
        with metadata_file.open('r') as f:
            return json.load(f)
    
    def _find_target_commit(self, metadata, commit_hash):
        """Find the target commit in metadata"""
        ...
        print(f"[GitBlend] Commit hash '{commit_hash}' not found in available commits")
        self.report({'ERROR'}, f"Commit {commit_hash[:8]} not found")
        return None
    ...
    # ============================================================================
    # SCENE RECONSTRUCTION ORCHESTRATION
    # ============================================================================
