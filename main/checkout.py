...
class GITBLEND_OT_Checkout(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if not gitblend_dir:
                self.report({'ERROR'}, "Could not determine .gitblend directory")
                return {'CANCELLED'}
            
            # Load metadata using repository service
            ...
            # Load target commit signature using repository service
            target_signature = RepositoryService.load_signature_file(target_commit['hash'])
            ...
            ExportService.cleanup_orphaned_data()
    ...
    def _load_full_commit(self, blend_file_path):
        """Load initial commit data (full state) using ExportService"""
        print(f"[GitBlend] Loading full commit state")
        ExportService.load_data_blocks_from_file(blend_file_path)
